...
- form_title
----------------------------------------- **/
const InputDropdown = BaseVue.extend({
  props: {
  ...
  watch: {
    datas: function(newValue, oldVal) {
      this.parseDatas(newValue);
    },
    "$props.select_value": function(newValue) {
      this.setValue(newValue);
    },
  },
  methods: {
    onconfig: function() {
      if (this.$props.select_value != null) {
        this.setValue(this.$props.select_value);
      }
      if (this.$props.inputChangeListener != null) {
        setTimeout(() => {
          this.setOnChangeListener(this.$props.inputChangeListener, null);
        }, 1000);
      }
    },
    oncomplete: async function() {
      await this.parseDatas(this.$props.datas);
    },
    ...
    loadDropdown: function() {
      ...
      self.inputDropdown = $("#" + (self.get("id") || self.get("random_id"))).dropdown(dropdownCOnfig);
      //}
    },
  ...
  render() {
    let { id, name, form_title, label, set_class, no_divider } = this.$props;

    let { random_id, _datas, saved_text, default_text } = this.get();

    return (
      <fragment>
        {form_title != null ? <h3 class="ui dividing header">{form_title}</h3> : no_divider == null ? null : <div class="ui divider"></div>}
        <div class="field">
          <label for="">{label}</label>
          <div class="ui left icon input">
            <i class="user icon"></i>
            <div class={"ui selection dropdown " + set_class} id={id || random_id}>
