let env = {
	APP_NAME : 'ARTYPLANET_PRODUCTION',
    DIST_URL: '/public',
    API_URL: 'https://service.artyplanet.io',
    API_ASSET_URL : 'https://service.artyplanet.io',
    HTTP_GENERATOR_URL : 'http://localhost:1084',
    BASE_PATH: '/',
    ARTYWIZ_HOST : 'https://artywiz.io',
    /* Ini mengikuti new Copy Plugin */
    ASSET : '/public',
    g_captcha_site_key : '6Lef5tUUAAAAANDvzGnYRQ-YdO8XjLPRrDyEnNsd',
    API_LIST : 'https://service.artyplanet.io/v1/member/routes',
    MAPBOX_ACCESS_TOKEN : 'pk.eyJ1IjoiYWthc2FkZXYiLCJhIjoiY2toNDNycTkxMDI2dTJzbWxubGxyOHlndiJ9.e9-jCWxYxk5m5A2nLymbTw'
};
module.exports = env;